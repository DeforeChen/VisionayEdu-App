...
-(void)show{
    ...
    self.SCRV.frame = CGRectMake((kViewWidth(self)-
                                  self.labelWidth-2*self.labelMid)/2,
    ...
    if(self.allcount>0){
        ...
        for (int i=0; i<self.allcount; i++) {
            ...
            if(i==0){
                lab.frame = CGRectMake(self.labelMid,
                                       0,
                                       self.labelWidth,
                                       self.maxHeight);
            ...
            if(self.showArr.count>0){
                NSString *text = (NSString *)self.showArr[i];
                lab.text       = [text capitalizedString];
            }else{
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(offset<=kViewMaxX(lastLab)+1){
        ...
        CGFloat countOffset = offset-count*(self.labelMid+self.labelWidth);
